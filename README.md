...
yarn mmo:sdk     # regenerate the ArtifactsMMO client stubs from the vendored spec
```
...
| `yarn mmo:spec`  | Refresh the vendored upstream spec `packages/server/spec/openapi.json` (network) |
| `yarn mmo:sdk`   | Regenerate `packages/server/generated-src/artifactsmmo/` from that spec |
| `yarn api:spec`  | Regenerate our own spec `packages/shared/spec/openapi.yaml` from the server's route schemas |
| `yarn api:client` | Regenerate the webapp's REST client from our spec |
| `yarn typecheck` | Typecheck every workspace                                              |
| `yarn test`      | Run the server's Vitest suite                                          |

After changing a route or shared schema: `yarn api:spec`, then `yarn api:client`,
...
- **`generated-src/`** — code generated by a pinned `@hey-api/openapi-ts`: the game-API
  ...
  linters skip it. After game updates run `yarn mmo:spec` then `yarn mmo:sdk`.
